...
class GenericModel(object):
    ...
    def set_image(self, img_path, img_shape):
        w, h, c = 1, 1, 3
        ...
        img = np.transpose(
            img, (0, 3, 1, 2)
        ).copy()  # (N, C, H, W) for the contiguous format
    ...
    def pad_model(self, config=None):
    ...
    def _add_operator(self, operator):
        ...
        if operator.name in self.operator_dict:
            raise ValueError("The operator already exists!: {}".format(operator.name))
    ...
    def print_conv_information(self, factor=1):
        ...
        for key in self.operator_dict:
            ...
            if isinstance(op, ConvOp):
                ...
                print(
                    "{}.{}.{}.{}".format(key, input_shape, kernel_shape, output_shape)
                )
