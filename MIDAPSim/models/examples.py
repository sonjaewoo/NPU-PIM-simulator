...
def srgan(input_shape=(1, 3, 512, 512)): # TODO: Not supported yet
    mb = ModelBuilder("srgan_{}x{}".format(input_shape[2], input_shape[3]))
    n = mb.set_input_tensor(tensor_shape=input_shape)
    n = mb.Conv(n, input_shape[1], 64, 3, 1, 'same')
    temp = n
    for _ in range(16):
        nn = mb.Conv(n, 64, 64, 3, 1, 'same')
        nn = mb.Conv(nn, 64, 64, 3, 1, 'same')
        n = mb.Sum(n, nn)
    n = mb.Conv(n, 64, 64, 3, 1, 'same')
    n = mb.Sum(n, temp)
    n = mb.Conv(n, 64, 256, 3, 1, 'same')
    n = mb.Reorg(n, 2)
    n = mb.Conv(n, 64, 256, 3, 1, 'same')
    n = mb.Reorg(n, 2)
    n = mb.Conv(n, 64, 3, 1, 1, 'same')
    return mb

def unet(input_shape=(1, 1, 384, 384), decompose=True):
...
def unet_small(input_shape=(1, 1, 32, 32), decompose=True):
    # Based on Official Tensorflow U-Net code (Vokeh)
    mb = ModelBuilder("unet_{}x{}".format(input_shape[2], input_shape[3]))
    x = mb.set_input_tensor(tensor_shape=input_shape)
    conv1 = mb.Conv(x, input_shape[1], 64, 3, 1, 'same')
    ...
    conv3 = mb.Conv(pool2, 128, 256, 3, 1, 'same')
    conv3 = mb.Conv(conv3, 256, 256, 3, 1, 'same')
    up8 = mb.Conv(conv3, 256, 128, 3, 1, 'same')
    up8 = mb.Upsample(up8, 2)
    if decompose:
        conv2 = mb.Conv(conv2, 128, 128, 3, 1, 'same', activation='Linear')
        conv8 = mb.Conv(up8, 128, 128, 3, 1, 'same', activation='Linear')
...
def vgg19_sr(input_shape = (1, 3, 256, 256)):
    mb = ModelBuilder("vgg19_sr_{}x{}".format(input_shape[2], input_shape[3]))
    x = mb.set_input_tensor(tensor_shape=input_shape)
    x = mb.Conv(x, input_shape[1], 64, 3, 1, 'same')
    for i in range(18):
        x = mb.Conv(x, 64, 64, 3, 1, 'same')
    x = mb.Conv(x, 64, 1, 3, 1, 'same')
    return mb
...
def test_network(input_shape = (1, 16, 15, 15)):
    mb = ModelBuilder("test_network")
    x = mb.set_input_tensor(tensor_shape=input_shape)
    conv1 = mb.Conv(x, input_shape[1], 64, 3, 2, 1) # (36, 36, 64)
    pool1 = mb.MaxPool(conv1, 3, 2, 1) # (18, 18, 64)
    # conv2 = mb.Conv(pool1, 64, 64, 3, 1, 'same') # (18, 18, 64)
    conv3 = mb.Conv(pool1, 64, 128, 1, 1) # (18, 18, 128)
    dwconv1 = mb.DWConv(conv3, 128, 3, 1, 'same') # (18, 18, 128)
    conv4 = mb.Conv(dwconv1, 128, 64, 1, 1) # (18, 18, 64)
    sum1 = mb.Mul(conv4, pool1) # (18, 18, 64)
    avgpool1 = mb.AvgPool(sum1, 2, 2) # (9, 9, 64)
    conv5 = mb.Conv(avgpool1, 64, 64, 3, 1, 'same')
    sum2 = mb.Sum(conv5, avgpool1) # (18, 18, 64)
    conv6 = mb.Conv(sum2, 64, 16, 1, 1)
    # Junk
    return mb