...
class VirtualTensor(object):
    ...
    def set_tensor(self, name, shape, orig_shape=None, init_shape=None, mapping_type='default', offset=None, scale=None, **kwargs):
        self.name = name
        self.shape = shape
    ...
    @property
    def is_vt(self):
        return not tuple(self.orig_shape) == tuple(self.shape)
...
class VInputTensor(VirtualTensor):
    ...
    def is_vt(self):
        scale_x, scale_y, scale_z = self.scale
        return any([self.orig_shape[-1] != self.shape[-1], scale_x != 1, scale_y != 1, scale_z != 1])

    @property
    def is_valid_type(self):
        return self.mapping_type == 'valid'

    @property
    def is_linear_type(self):
        return self.is_vt and not self.is_valid_type

class VOutputTensor(VirtualTensor):
    mapping_algorithms = ['default', 'linear', 'shuffle', 'wmem']
    ...
    def get_loc(self, loc):
        ...
        if self.mapping_type == 'default':
            locs = [loc]
        if self.mapping_type in ['linear', 'wmem']:
            scale_x, scale_y, scale_z = self.scale
